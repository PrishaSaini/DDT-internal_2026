...
class CatalogueScreen:
    ...
    def __init__(self,root,user,item):
                ...
                tk.Label(self.frame, text=fmt_price["Price"], font=("Arial",14)).pack(pady=10)
    ...
    def confirm(self):
                        ...
                        if not pickup:
                            ...
                            return
                        if not reserve-item(self.item["ItemID"], self.user["id"], self.payment.get(),pickup):
                            messagebox.showerror("Error", "Sorry, that item is no longer available.")
                            self.back()
                            return
                        messagebox.showerror("Reserved", "Item reserved. Check My Orders for the details.")
                        self.back()
                        return
